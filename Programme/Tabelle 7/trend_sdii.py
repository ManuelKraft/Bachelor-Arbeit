...
list_x = []
list_y = []
f = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/World/SDII/SDII(3).csv", 'r')
t = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/World/SDII/SDII_trend_all.csv", 'w')
t.write("stationid,absolute_increase_per_year, relative_increase_per_year_in_% (absolute_increase / middle_value)\n")
t2 = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/World/SDII/SDII_trend_recent.csv", 'w')
...
t3 = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/World/SDII/SDII_trend_comparison.csv", 'w')
...
while row:
    ...
    while stationid == getStation(row):
        list_y.append(float(get_y(row))/100)
    ...
    if len(list_x) >= 24 and list_y.count(0) != len(list_y):
        ...
        t.write(stationid + ',' + str(round(list_trend[0], 3)) + ',' + str(round(list_trend[1]*100, 2)) + '\n')
    if enough_recent_data(list_x) == True and create_recent_list(list_x, list_y)[1].count(0) != len(create_recent_list(list_x, list_y)[1]):
        ...
        t2.write(stationid + ',' + str(round(list_recent_trend[0], 3)) + ',' + str(round(list_recent_trend[1]*100, 2)) + '\n')
    list_y = []
    list_x = []
...
while i < len(data_all[0]):
    ...
    while j < len(data_recent[0]):
        if data_recent[0][j] == data_all[0][i]:
            t3.write(data_all[0][i] + ',' + str(round(data_recent[1][j][0] - data_all[1][i][0], 3)) + ',' + str(round((data_recent[1][j][0] - data_all[1][i][0])/data_all[1][i][2]*100, 2)) + '\n')
            break
        j += 1
    i += 1

f = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/Germany/sdii/sdii_trend_all.csv", 'r')
f2 = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/Germany/sdii/sdii_trend_recent.csv", 'r')
f3 = open("/mnt/c/Uni/Bachelor-Arbeit/prcp_Indices/Germany/sdii/sdii_trend_comparison.csv", 'r')
row = f.readline()
row2 = f2.readline()
row3 = f3.readline()
while row:
